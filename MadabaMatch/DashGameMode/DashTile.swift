...
import SpriteKit
import AudioToolbox
class DashTile:NSObject, NSSecureCoding{
    ...
    convenience init(copy:DashTile) {
    ...
    func copy()->DashTile{
        return DashTile.init(copy: self)
    }
    func copyPosition(of tile:DashTile){
        self.position=tile.position
    }
    func switchPosition(with tile:DashTile){
        if(!animating){
